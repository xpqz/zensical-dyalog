...
!!! Info "Information"
    Under normal circumstances, APL thread 0 is never used in the Dyalog DLL. It is only ever used if, during debugging, the APL programmer explicitly changes to thread 0 by executing `)TID 0` and then runs an expression.
...
In these cases, all calls to Microsoft .NET are initiated by Dyalog. However, these calls might result in calls being made back from .NET into APL.

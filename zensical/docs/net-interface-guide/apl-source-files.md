...
Assignment statements are used to configure system variables, such as [⎕ML](../language-reference-guide/system-functions/ml.md), [`⎕IO`](../language-reference-guide/system-functions/io.md), [`⎕USING`](../language-reference-guide/system-functions/using.md), and arbitrary APL variables. For example:
```apl
      ⎕ML←2
      ⎕IO←0
      ⎕USING∪←⊂'System.Data'
...
At least one namespace must be specified in an APL Source file. Namespaces are specified in an APL Source file using the `:Namespace` and `:EndNamespace` statements. Although you can use [`⎕NS`](../language-reference-guide/system-functions/ns.md) and [`⎕CS`](../language-reference-guide/system-functions/cs.md) within functions inside an APL Source file, you should not use these system functions outside function bodies; such use is not prevented, but the results will be unpredictable.

...
When you define a class, you list the interfaces which it supports following a colon after the class name. The value of [`⎕USING`](../../language-reference-guide/system-functions/using.md) (possibly set by `:Using`) is used to locate interface names.

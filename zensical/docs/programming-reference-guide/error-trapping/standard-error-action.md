...
1. The error message, preceded by the symbol `⍎` if the error occurred while evaluating the _execute_ function.
2. The statement in which the error occurred (or expression being evaluated by the _execute_ function), preceded by the name of the function and line number where execution is suspended unless the state indicator has been cut back to immediate execution mode.  If the state indicator has been cut back because of a locked function in execution, the displayed statement is that from which the locked function was invoked.

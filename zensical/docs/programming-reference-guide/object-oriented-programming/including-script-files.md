...
If no `path` is specified, the path is taken to be relative to the current script file or, if in a workspace script, the current working directory. A leading `'./'` or `'.\'` in `path` is not allowed (to avoid any potential confusion with "current directory").
...
In version {{ version_majmin }} `⍝!:require` is a synonym for `:Require`.  This allows the user to create scripts which can be used in multiple versions of Dyalog; in Dyalog v14.1 and earlier, SALT parses `⍝!:require` statements and loads the appropriate files, in Dyalog v{{ version_majmin }} it is the interpreter that loads the file named in `⍝!:require` statements. Dyalog intends to remove support for the `⍝!:require` statement from the interpreter in a future version. Unlike `:Require`, `⍝!:require` can appear within code.

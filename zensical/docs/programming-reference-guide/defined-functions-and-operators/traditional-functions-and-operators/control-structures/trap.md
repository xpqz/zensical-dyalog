...
`ecode` is an integer scalar or vector containing the list of event codes which are to be *handled* during execution of the segment of code between the `:Trap` and `:End[Trap]` statements. Event codes 0 and 1000 are wild cards that mean *any* event code in a given range. See [APL Error Messages](../../../error-messages/apl-errors.md).

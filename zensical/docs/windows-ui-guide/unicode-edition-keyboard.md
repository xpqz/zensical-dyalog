...
For any other Input Language the American English translate table is selected. Some Input Languages are defined to be *substitutes* for other Input Languages; for example, Australian English Input is a substitute for American English Input, and Austrian German Input is a substitute for German German Input. In these cases, the IME will install the appropriate translate table. It is also possible to create support for new languages or to modify the existing support, by altering the [input translate table](ime-configuration.md#input-translate-table).

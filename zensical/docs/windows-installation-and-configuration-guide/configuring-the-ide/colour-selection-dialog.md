...
|Session Input|Check this box if you want to enable syntax colouring in the Session window. The colour scheme used for the Session can differ from the colour scheme selected for Edit windows and is specified by the *Session Colour Scheme* box on the *Session/Trace* tab.|
|Only current input line|This option only applies if Session syntax colouring is enabled. Check this box if you want syntax colouring to apply only to the current input line. Clear this box, if you want to apply syntax colouring to all the input lines in the current Session window. Syntax colouring of input lines is not remembered in the Session log, so input lines from previous sessions do not have syntax colouring.|

...
In previous versions of Dyalog, indexed Properties of COM objects were exposed as Methods and for backwards compatibility this behaviour can be retained by setting [`⎕WX`](../../language-reference-guide/system-functions/wx.md) to `0` or `1` (the default value is `3`).
...
However, in this example `BIKE`, `FRONT`, and `REAR` are all top-level namespaces; a proper parent/child representation can be achieved by making `FRONT` and `REAR` child namespaces of `BIKE`, for example:
```apl

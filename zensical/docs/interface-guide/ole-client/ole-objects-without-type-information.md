# Using OLE Objects without Type Information
...
This means that if you know, from its documentation or another source, that a COM object provides a certain Method or Property, you can, therefore, access that member by calling a function of that name in the OLEClient namespace. Any parameters you pass will be returned in the result, because APL assumes that all parameters are input/output. Furthermore, APL will be unable to check the validity of the parameters you specify because it does not know what data types are expected.

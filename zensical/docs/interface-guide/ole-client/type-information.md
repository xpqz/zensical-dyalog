...
Then, we can obtain information about a particular event (or events) by invoking a GetEventInfo method. In the case of the Windows Media Control, it is necessary to query the instance of the control (`f.wmp`) as opposed to the instance of the OCXClass (`wmp`). For example, you can ask it about its MouseDown event. The result is a vector, each element of which is a 2-element vector of character vectors.
```apl

...
In addition to providing a means for exchanging **data**, DDE provides a mechanism for one application to instruct another application to **execute** a command. This is implemented by sending a DDE_EXECUTE message. It is important to understand that the effect of the command is local to the application in which it is executed, and that the recipient of the message does **not** return a result to the originating application. It does **not** work like the APL _execute_ function.

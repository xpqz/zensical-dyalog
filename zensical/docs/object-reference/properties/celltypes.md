...
For example, if CellTypes[1;1] is 3, the first cell in the [Grid](../objects/grid.md) is displayed using the foreground colour specified by the 3rd element of [FCol](fcol.md), the background colour specified by the 3rd element of [BCol](bcol.md), and so forth. However, scalar property arrays are extended if necessary. Therefore if you require 5 different foreground colours but only one background colour, [BCol](bcol.md) need specify only a single colour.

...
The value of the date or time is represented by the [DateTime](datetime.md) property. All validation is performed by the object itself, and it is impossible to enter an invalid value.
...
For [Edit](../objects/edit.md), [Label](../objects/label.md) and [Spinner](../objects/spinner.md) objects, FieldType can only be specified when you create an object using `⎕WC`.

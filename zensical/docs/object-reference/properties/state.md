...
If the [Style](style.md) property is `'Radio'` or `'Check'`, 0 means "not selected" and 1 means "selected". Only one of a group of buttons with [Style](style.md) `'Radio'` that share the same parent can have State 1. Setting State to 1 automatically deselects all the others in the group.

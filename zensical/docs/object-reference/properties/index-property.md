...
For a [List](../objects/list.md) and a [Combo](../objects/combo.md) with [Style](style.md) `'Simple'`, this property specifies the position of the data in the list box as a positive integer value. If Index has the value "n", it means that the "nth" item in [Items](items.md) is displayed on the top line in the list box. The value of Index is dependent upon the value of `⎕IO`. Index for a [Combo](../objects/combo.md) or [List](../objects/list.md) cannot be set using [`⎕WC`](../../language-reference-guide/system-functions/wc.md). The value of Index in a [Combo](../objects/combo.md) with a drop-down list box ([Style](style.md) `'Drop'` or `'DropEdit'`) is always equal to `⎕IO`.

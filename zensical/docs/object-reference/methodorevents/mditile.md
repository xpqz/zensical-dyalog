...
Because there are restrictions concerning the minimum height and width of a window, Microsoft Windows does not necessarily respond as requested. If the [MDIClient](../objects/mdiclient.md) is itself of insufficient size, or if it contains a large number of child [Form](../objects/form.md)s, Windows may choose to tile the [Form](../objects/form.md)s in a row when a column was specified or vice versa. It may also choose to ignore the event entirely.

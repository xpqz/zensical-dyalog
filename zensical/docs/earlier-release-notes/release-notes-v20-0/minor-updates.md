...
[APL Exit Codes](https://docs.dyalog.com/20.0/windows-installation-and-configuration-guide/apl-exit-codes/)  
Two exit codes have been added. The interpreter returns `12` if the HMON ([Dyalog Health Monitor](https://github.com/Dyalog/HMON)) library cannot be loaded, and `13` if that library is loaded but cannot be initialised. An application that returns its own exit code from [`⎕OFF`](https://docs.dyalog.com/20.0/language-reference-guide/system-functions/off/) should, therefore, use a value greater than `13`.

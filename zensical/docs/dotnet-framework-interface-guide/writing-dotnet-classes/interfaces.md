...
The [`TemperatureConverterCtl2`](../writing-custom-aspnet-controls/example-the-temperatureconverterctl2-control.md) custom control derives from <code class="language-nonAPL">System.Web.UI.Control</code>. The first line of this class definition reads:
```apl
...
Following the colon, the first name is the base class. After the (optional) base class name is the list of interfaces that are implemented. The <code class="language-nonAPL">TemperatureControlCtl2</code> custom control implements two interfaces called <code class="language-nonAPL">IPostBackDataHandler</code> and <code class="language-nonAPL">IPostBackEventHandler</code>. These interfaces are required for a custom control that intends to render the HTML for its own form elements in a web page. They define certain methods that get called at the appropriate time by the page framework when a web page is constructed for the browser. It is, therefore, essential that the class implements all the methods specified by the interface, even if they do nothing.

The base class, <code class="language-nonAPL">System.Web.UI.Control</code>, defines an optional interface called <code class="language-nonAPL">INamingContainer</code>. A class based on <code class="language-nonAPL">Control</code> that implements <code class="language-nonAPL">INamingContainer</code> specifies that its child controls are to be assigned unique ID attributes within an entire application. This is a marker interface with no methods or properties defined for it.
